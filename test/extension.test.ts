...
//   it('castas template', testTemplate('expr', 'castas', '(expr as )'))
  ...
  describe('custom template tests', () => {
    ...
    //   config.update('languages', ["postfix"], true).then(() => done(), err => done(err))
    })

    after(done => {
      config.update('customTemplates', undefined, true).then(() => done(), err => done(err))
    //   config.update('languages', ["python"], true).then(() => done(), err => done(err))
    })

    it('identifier', testTemplate('expr.', 'nn', 'expr is not None'))
